...
With `external_inference_provider=stitch`, the API process instead uses
`ExternalStitchInferenceClient`. It sends SGLang `/generate` requests carrying
the sampler checkpoint's exact per-model weight version and accepts only
responses stamped with that version by the Stitch sidecar.
...
The external Stitch path replaces this push with a PEFT adapter export into a
run-partitioned bulletin board. Rollout replicas pull the published version;
SkyRL does not create or address local inference engines in this mode.
...
External Stitch rollouts still write the PEFT adapter into the bulletin board
because pull-based replicas require a durable artifact. They skip only the
separate user-visible sampler archive.
